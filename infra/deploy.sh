...
gcloud run jobs deploy "$JOB_NAME" \
  ...
  --max-retries 1 \
  --task-timeout 120s \
  --set-env-vars "TABCLOSE_BUCKET=$BUCKET,TABCLOSE_USE_FIRESTORE=1,TABCLOSE_SERVICE_NAME=tabclose-demo-target,TABCLOSE_TARGET_URL=${DEMO_TARGET_URL}/health,TABCLOSE_OBSERVER_B_URL=${OBSERVER_B_URL},TABCLOSE_WINDOW_MINUTES=2,GOOGLE_GENAI_USE_VERTEXAI=TRUE,GOOGLE_CLOUD_PROJECT=$PROJECT,GOOGLE_CLOUD_LOCATION=$REGION_A"
